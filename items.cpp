...
ITEM* append(ITEM * head)///在尾部再加一个item
{
    ///创建新结点
    ITEM* new_item;
    ...
    previous->Next_Ptr = new_item;
    return head;
}

void update_all(ITEM * head,int xPlane,int yPlane,int &life)
{
         ...
         previous = current, current = current->Next_Ptr)   ///这里相当于循环变量往后迭代一个
    {
        ...
        if (current->x <= xPlane+A_plane&& (yPlane - current->y >= -A_plane && yPlane - current->y <= Item_Height))
        {///碰撞检测
           bumpFlag = true;
